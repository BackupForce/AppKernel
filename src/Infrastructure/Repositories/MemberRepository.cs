...
internal sealed class MemberRepository(
    ...
    IDbConnectionFactory dbConnectionFactory) : IMemberRepository
{
    ...
    public async Task<int> GetNextMemberNoSequenceAsync(Guid tenantId, CancellationToken cancellationToken = default)
    {
        const string sql =
            """
        ...
        CommandDefinition command = new(sql, new { TenantId = tenantId }, cancellationToken: cancellationToken);
