...
## 主要流程
1. **發券/領券**：活動期間建立 Ticket，並在發券時決定 Primary Draw（只記錄於 Ticket）。
2. **提交號碼**：會員對票券提交一組號碼，下注當下建立單筆 TicketDraw，並依 `Draw.IsWithinSalesWindow` 標記為 Active。
...
- **TicketDraw**：以 TicketId + DrawId 做唯一鍵避免重複參與期數，僅在下注時建立單筆記錄。
